...
namespace ImTricks {
	...
	namespace Animations {
		...
		std::unordered_map<const char*, int> valuesMapInt;
		...
		std::unordered_map<const char*, float> valuesMapFloat;
		...
		std::unordered_map<const char*, ImVec2> valuesMapVec;
		ImVec2 FastImVec2Lerp(const char* identifier, bool state, ImVec2 min, ImVec2 max, float speed) {

			auto value = valuesMapVec.find(identifier);

			if (value == valuesMapVec.end()) {
				valuesMapVec.insert({ identifier, min });
				value = valuesMapVec.find(identifier);
			}
	...
	namespace NotifyManager {
		...
		void HandleNotifies(ImDrawList* draw) {
			...
			auto DrawNotify = [&draw, &NotifyPos](int id, NotifyStruct notify) {

				const auto NotifyEndPos = NotifyPos + ImVec2(300, 30);
				float trickFloat = ImTricks::Animations::FastFloatLerp(std::to_string(id).c_str(), notify.time < (GetTickCount64() + 500), 0.f, 1.f, 0.05f);

				draw->AddRectFilled(NotifyPos, NotifyEndPos, ImGui::GetColorU32(ImGuiCol_PopupBg, trickFloat), ImGui::GetStyle().PopupRounding);
			...
			for (int i = 0; i < notifies.size(); i++)
			{
				...
				if (current_notify.time < GetTickCount64())
				{
					...
					Animations::valuesMapFloat.erase(std::to_string(i).c_str());

					continue;
				}
				DrawNotify(i, current_notify);
			}
		}
	}
